-- sortedtree.hs
-- Jeremy.Singer@glasgow.ac.uk, 2074813f@student.gla.ac.uk
...
addInOrder :: Tree -> Int -> Tree
-- add a new element to a tree in order
addInOrder Leaf x = Node x Leaf Leaf
addInOrder (Node value left right) x
    | x < value = Node value (addInOrder left x) right
    | x >= value = Node value left (addInOrder right x)
